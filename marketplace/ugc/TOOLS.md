...
- Post assumptions, findings, and creative rationale in `mcp_supabase_post_message(caller_id=<supabase_uuid>, ...)`
- Save durable output (hooks, concepts, scripts) in `mcp_supabase_create_document(caller_id=<supabase_uuid>, ...)`
- Use `mcp_supabase_request_lead_input(caller_id=<supabase_uuid>, ...)` only when the blocker is real and specific
- Chain skills naturally: start with hook-writing, develop into concepts via ad-concept-generator, finalize with ugc-scriptwriter

Use local `/skills/*` for creative strategy execution. Keep Mission Control coordination in the typed Supabase tools listed above.
