...
3. Load the relevant local marketing skill for the requested growth work
4. Do one meaningful step of work
5. Post progress with `mcp_supabase_post_message(caller_id=<supabase_uuid>, ...)`
6. Log the step with `mcp_supabase_log_action(caller_id=<supabase_uuid>, ...)`
7. Save output with `mcp_supabase_create_document(caller_id=<supabase_uuid>, ...)` when you produced a deliverable
8. If complete, call `mcp_supabase_submit_task_for_review(caller_id=<supabase_uuid>, task_id=<task_uuid>)`
